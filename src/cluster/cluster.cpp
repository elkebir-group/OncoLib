...
void Cluster::clusterCliques(double beta)
{
  Graph G;
  ...
  Graph::NodeMap<bool> vertexMap(G, true);

  int index = 0;
  lemon::FilterNodes<Graph> subG(G, vertexMap);
  while (lemon::countNodes(subG) > 0)
  {
    Graph GG;
    Graph::NodeMap<Graph::Node> toOrgNode(GG, lemon::INVALID);
    lemon::graphCopy(subG, GG).nodeCrossRef(toOrgNode).run();
    
    gm::BronKerbosch bk(GG, -1);
    bk.run(gm::BronKerbosch::BK_PIVOT_DEGENERACY);
    bk.sortBySize();
    
    assert(bk.getNumberOfMaximalCliques() > 0);
    const gm::BronKerbosch::NodeVector& C = bk.getCliques()[0];
    for (Graph::Node v : C)
    {
      Graph::Node vv = toOrgNode[v];
      vertexMap[vv] = false;
      compMap[vv] = index;
    }
    ++index;
