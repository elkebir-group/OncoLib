...
int Simulation::run(const std::string& outputDirectory,
                    const std::string& filenameColorMap,
                    int seed,
                    ...
                    bool verbose)
{
  ...
  Simulation::Pattern migPattern = static_cast<Simulation::Pattern>(migrationPattern);
  
  if (maxNrTrials == -1)
  {
    g_rng = std::mt19937(seed);
    Simulation simulation(carryingCapacity,
                          migrationRate,
                          mutationRate,
                          driverRate,
                          mutFreqThreshold,
    ...
    if (simulation.simulate(verbose))
    {
      if (filenameColorMap.empty())
      {
        colorMap = simulation.getCloneTree().generateColorMap();
      }
      if (outputDirectory.empty())
      {
        std::cout << simulation.getObservedCloneTree();
      }
      else
      {
...
void Simulation::output(const Simulation& simulation,
...
//  outT_dot.close();
//  
//  snprintf(buf, 1024, "%s/T_all_seed%d.tree", outputDirectory.c_str(), seed);
...
//  outGG_dot.close();
  ...
  snprintf(buf, 1024, "%s/G_seed%d.graph", outputDirectory.c_str(), seed);
...
bool Simulation::simulate(bool verbose)
{
  ...
  while (_nrExtantCells > 0)
  {
    if (_nrActiveAnatomicalSites > _maxNrAnatomicalSites)
    {
      if (verbose)
      {
        std::cerr << "Maximum number of active anatomical sites reached" << std::endl;
      }
      break;
    }
    ...
    {
      std::cerr << "Generation: " << _generation
...
void Simulation::constructCloneTree()
{
  ...
  for (NodeIt v(_pCloneT->tree()); v != lemon::INVALID; ++v)
  {
    ...
    {
      DoubleVector U_v(1, cloneFrequency[vv]);
...
bool Simulation::constructSampledCloneTree()
{
  ...
  _pCloneTT = new MutCloneTree(newT, newRoot, label, anatomicalSiteStr);
